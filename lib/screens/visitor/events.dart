...
class _EventsState extends State<Events> {
  ...
  Future<void> _filterEventsByDate(DateTime date) async {
    try {
      final uri = Uri.parse('http://192.168.1.36:5000/event/getEventsByDate/${DateFormat('yyyy-MM-dd').format(date)}');
  ...
  Future<List<Event>> fetchEvents() async {
    final uri = Uri.parse('http://192.168.1.36:5000/event/getEvent');
    final response = await http.get(uri);
