...
Future<void> _deleteNews(BuildContext context, String id, VoidCallback onSuccess) async {
  final uri = Uri.parse('http://192.168.1.36:5000/news/deleteNews/$id');
...
class _AdminNewsWidgetState extends State<AdminNewsWidget> {
  ...
  Future<void> _loadNewsId() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.1.36:5000/news/getNewsById/${widget.news.id}'),
        headers: {
          'Content-Type': 'application/json',
        },
      );
  ...
  Future<void> _fetchComments() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.1.36:5000/comment/getAllComments/${widget.news.id}'),
        headers: {
          'Content-Type': 'application/json',
        },
      );
  ...
  Future<void> _deleteComment(String commentId) async {
    try {
      final uri = Uri.parse('http://192.168.1.36:5000/comment/deleteComment/$commentId');
  ...
  Widget build(BuildContext context) {
    final imageUrl = 'http://192.168.1.36:5000/images/${widget.news.news_photo}'; // URL de l'image
